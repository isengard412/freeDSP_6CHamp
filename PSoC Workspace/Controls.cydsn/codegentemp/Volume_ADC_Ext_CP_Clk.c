/*******************************************************************************
* File Name: Volume_ADC_Ext_CP_Clk.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: Volume_ADC_Ext_CP_Clk_Start
********************************************************************************
*
* Summary:
...
void Volume_ADC_Ext_CP_Clk_Start(void) 
{
    ...
    Volume_ADC_Ext_CP_Clk_CLKEN |= Volume_ADC_Ext_CP_Clk_CLKEN_MASK;
	Volume_ADC_Ext_CP_Clk_CLKSTBY |= Volume_ADC_Ext_CP_Clk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Volume_ADC_Ext_CP_Clk_Stop
********************************************************************************
*
* Summary:
...
void Volume_ADC_Ext_CP_Clk_Stop(void) 
{
    ...
    Volume_ADC_Ext_CP_Clk_CLKEN &= (uint8)(~Volume_ADC_Ext_CP_Clk_CLKEN_MASK);
	Volume_ADC_Ext_CP_Clk_CLKSTBY &= (uint8)(~Volume_ADC_Ext_CP_Clk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Volume_ADC_Ext_CP_Clk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Volume_ADC_Ext_CP_Clk_CLKEN & Volume_ADC_Ext_CP_Clk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Volume_ADC_Ext_CP_Clk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Volume_ADC_Ext_CP_Clk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Volume_ADC_Ext_CP_Clk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Volume_ADC_Ext_CP_Clk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Volume_ADC_Ext_CP_Clk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Volume_ADC_Ext_CP_Clk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Volume_ADC_Ext_CP_Clk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Volume_ADC_Ext_CP_Clk_GetSourceRegister();
    uint16 oldDivider = Volume_ADC_Ext_CP_Clk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Volume_ADC_Ext_CP_Clk_CLKEN & Volume_ADC_Ext_CP_Clk_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Volume_ADC_Ext_CP_Clk_DIV_PTR, clkDivider);
                Volume_ADC_Ext_CP_Clk_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Volume_ADC_Ext_CP_Clk_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Volume_ADC_Ext_CP_Clk_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Volume_ADC_Ext_CP_Clk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Volume_ADC_Ext_CP_Clk_DIV_PTR, clkDivider);
				Volume_ADC_Ext_CP_Clk_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Volume_ADC_Ext_CP_Clk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Volume_ADC_Ext_CP_Clk_DIV_PTR);
}


/*******************************************************************************
* Function Name: Volume_ADC_Ext_CP_Clk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Volume_ADC_Ext_CP_Clk_MOD_SRC |= modeBitMask & (uint8)Volume_ADC_Ext_CP_Clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: Volume_ADC_Ext_CP_Clk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Volume_ADC_Ext_CP_Clk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Volume_ADC_Ext_CP_Clk_MODE_MASK));
}


/*******************************************************************************
* Function Name: Volume_ADC_Ext_CP_Clk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Volume_ADC_Ext_CP_Clk_MOD_SRC & (uint8)(Volume_ADC_Ext_CP_Clk_MODE_MASK);
}


/*******************************************************************************
* Function Name: Volume_ADC_Ext_CP_Clk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Volume_ADC_Ext_CP_Clk_GetDividerRegister();
    uint8 oldSrc = Volume_ADC_Ext_CP_Clk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Volume_ADC_Ext_CP_Clk_MOD_SRC |= CYCLK_SSS;
        Volume_ADC_Ext_CP_Clk_MOD_SRC =
            (Volume_ADC_Ext_CP_Clk_MOD_SRC & (uint8)(~Volume_ADC_Ext_CP_Clk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Volume_ADC_Ext_CP_Clk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Volume_ADC_Ext_CP_Clk_MOD_SRC & Volume_ADC_Ext_CP_Clk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Volume_ADC_Ext_CP_Clk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Volume_ADC_Ext_CP_Clk_PHASE = clkPhase & Volume_ADC_Ext_CP_Clk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Volume_ADC_Ext_CP_Clk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Volume_ADC_Ext_CP_Clk_PHASE & Volume_ADC_Ext_CP_Clk_PHASE_MASK;
}
