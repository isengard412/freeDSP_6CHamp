...
#if (COM_MIDI_EXT_MODE >= COM_ONE_EXT_INTRF)
    /* The Size of the MIDI messages (MIDI Table 4-1) */
    static const uint8 CYCODE COM_MIDI_SIZE[] = {
...
#endif  /* COM_MIDI_EXT_MODE >= COM_ONE_EXT_INTRF */
...
***************************************/
...
#if (COM_MIDI_IN_BUFF_SIZE > 0)
    #if (COM_MIDI_IN_BUFF_SIZE >= 256)
        ...
        * USBMIDI_MIDI_EP_Init() function.*/
        volatile uint16 COM_midiInPointer;                            /* Input endpoint buffer pointer */
    #else
        volatile uint8 COM_midiInPointer;                             /* Input endpoint buffer pointer */
    #endif /* (COM_MIDI_IN_BUFF_SIZE >= 256) */
    ...
    uint8 COM_midiInBuffer[COM_MIDI_IN_BUFF_SIZE];       /* Input endpoint buffer */
#endif /* (COM_MIDI_IN_BUFF_SIZE > 0) */

#if (COM_MIDI_OUT_BUFF_SIZE > 0)
    ...
    volatile uint8 COM_midi_out_ep;                                   /* Output endpoint number */
    ...
    uint8 COM_midiOutBuffer[COM_MIDI_OUT_BUFF_SIZE];     /* Output endpoint buffer */
#endif /* (COM_MIDI_OUT_BUFF_SIZE > 0) */

#if (COM_MIDI_EXT_MODE >= COM_ONE_EXT_INTRF)

    static COM_MIDI_RX_STATUS COM_MIDI1_Event;            /* MIDI RX status structure */
    static volatile uint8 COM_MIDI1_TxRunStat;                         /* MIDI Output running status */
    ...
    volatile uint8 COM_MIDI1_InqFlags;                                 /* Device inquiry flag */

    #if (COM_MIDI_EXT_MODE >= COM_TWO_EXT_INTRF)
        static COM_MIDI_RX_STATUS COM_MIDI2_Event;        /* MIDI RX status structure */
        static volatile uint8 COM_MIDI2_TxRunStat;                     /* MIDI Output running status */
        /** See description of \ref COM_MIDI1_InqFlags*/
        volatile uint8 COM_MIDI2_InqFlags;                             /* Device inquiry flag */
    #endif /* (COM_MIDI_EXT_MODE >= COM_TWO_EXT_INTRF) */
#endif /* (COM_MIDI_EXT_MODE >= COM_ONE_EXT_INTRF) */
...
#if (COM_ENABLE_MIDI_API != 0u)
/*******************************************************************************
* Function Name: COM_MIDI_Init
****************************************************************************//**
*
...
* \globalvars
*
*  \ref COM_midiInBuffer: This buffer is used for saving and combining
*    the received data from UART(s) and(or) generated internally by
*    PutUsbMidiIn() function messages. COM_MIDI_IN_EP_Service()
*    function transfers the data from this buffer to the PC.
*
*  \ref COM_midiOutBuffer: This buffer is used by the
*    COM_MIDI_OUT_Service() function for saving the received
*    from the PC data, then the data are parsed and transferred to UART(s)
*    buffer and to the internal processing by the
*
*  \ref COM_callbackLocalMidiEvent function.
*
*  \ref COM_midi_out_ep: Used as an OUT endpoint number.
*
*  \ref COM_midi_in_ep: Used as an IN endpoint number.
*
*   \ref COM_midiInPointer: Initialized to zero.
*
* \sideeffect
...
void COM_MIDI_Init(void) 
{
   ...
   COM_midiInPointer = 0u;
        ...
        COM_LoadInEP(COM_midi_in_ep, COM_midiInBuffer,
                                                               COM_MIDI_IN_BUFF_SIZE);
    #endif  /* (COM_MIDI_IN_BUFF_SIZE > 0) */

    #if (COM_MIDI_OUT_BUFF_SIZE > 0)
        /* Provide buffer for OUT endpoint. */
        (void)COM_ReadOutEP(COM_midi_out_ep, COM_midiOutBuffer,
                                                                       COM_MIDI_OUT_BUFF_SIZE);
    #endif /* (COM_MIDI_OUT_BUFF_SIZE > 0) */
#endif /* (COM_EP_MANAGEMENT_DMA_AUTO) */

#if (COM_MIDI_OUT_BUFF_SIZE > 0)
    COM_EnableOutEP(COM_midi_out_ep);
#endif /* (COM_MIDI_OUT_BUFF_SIZE > 0) */

    /* Initialize the MIDI port(s) */
#if (COM_MIDI_EXT_MODE >= COM_ONE_EXT_INTRF)
    COM_MIDI_InitInterface();
#endif /* (COM_MIDI_EXT_MODE >= COM_ONE_EXT_INTRF) */
}
...
#if (COM_MIDI_OUT_BUFF_SIZE > 0)
    /*******************************************************************************
    * Function Name: COM_MIDI_OUT_Service
    ****************************************************************************//**
    *
    ...
    * \globalvars
    *
    *  \ref COM_midiOutBuffer: Used as temporary buffer between USB
    ...
    *
    *******************************************************************************/
    void COM_MIDI_OUT_Service(void) 
    {
    #if (COM_MIDI_OUT_BUFF_SIZE >= 256)
        uint16 outLength;
        uint16 outPointer;
    #else
        uint8 outLength;
        uint8 outPointer;
    #endif /* (COM_MIDI_OUT_BUFF_SIZE >= 256) */

        /* Service the USB MIDI output endpoint. */
        if (COM_OUT_BUFFER_FULL == COM_GetEPState(COM_midi_out_ep))
        {
        #if (COM_MIDI_OUT_BUFF_SIZE >= 256)
            outLength = COM_GetEPCount(COM_midi_out_ep);
        #else
            outLength = (uint8)COM_GetEPCount(COM_midi_out_ep);
        #endif /* (COM_MIDI_OUT_BUFF_SIZE >= 256) */

        #if (!COM_EP_MANAGEMENT_DMA_AUTO)
            #if (COM_MIDI_OUT_BUFF_SIZE >= 256)
                outLength = COM_ReadOutEP(COM_midi_out_ep,
                                                       COM_midiOutBuffer, outLength);
            #else
                outLength = (uint8)COM_ReadOutEP(COM_midi_out_ep,
                                                              COM_midiOutBuffer, (uint16) outLength);
            #endif /* (COM_MIDI_OUT_BUFF_SIZE >= 256) */

            #if (COM_EP_MANAGEMENT_DMA_MANUAL)
                /* Wait until DMA complete transferring data from OUT endpoint buffer. */
                while (COM_OUT_BUFFER_FULL == COM_GetEPState(COM_midi_out_ep))
                {
                }
            ...
            if (outLength >= COM_EVENT_LENGTH)
            {
                ...
                #if (COM_MIDI_EXT_MODE >= COM_ONE_EXT_INTRF)
                    if ((COM_midiOutBuffer[outPointer] & COM_CABLE_MASK) ==
                         COM_MIDI_CABLE_00)
                    {
                        COM_MIDI1_ProcessUsbOut(&COM_midiOutBuffer[outPointer]);
                    }
                    else if ((COM_midiOutBuffer[outPointer] & COM_CABLE_MASK) ==
                             COM_MIDI_CABLE_01)
                    {
                    #if (COM_MIDI_EXT_MODE >= COM_TWO_EXT_INTRF)
                         COM_MIDI2_ProcessUsbOut(&COM_midiOutBuffer[outPointer]);
                    #endif /*  COM_MIDI_EXT_MODE >= COM_TWO_EXT_INTRF */
                    }
                    else
                    {
                        ...
                        #ifdef COM_MIDI_OUT_EP_SERVICE_CALLBACK
                            COM_MIDI_OUT_EP_Service_Callback();
                        #endif /* COM_MIDI_OUT_EP_SERVICE_CALLBACK */
                    }
                #endif /* (COM_MIDI_EXT_MODE >= COM_ONE_EXT_INTRF) */

                    /* Process any local MIDI output functions */
                    COM_callbackLocalMidiEvent(COM_midiOutBuffer[outPointer] & COM_CABLE_MASK,
                                                            &COM_midiOutBuffer[outPointer + COM_EVENT_BYTE1]);
                    outPointer += COM_EVENT_LENGTH;
                }
            }
...
#if (COM_MIDI_IN_BUFF_SIZE > 0)
    /*******************************************************************************
    * Function Name: COM_MIDI_IN_EP_Service
    ****************************************************************************//**
    *
    ...
    *  COM_MIDI_IN_Service() and from the
    *  COM_PutUsbMidiIn() function.
    *
    * \globalvars
    *  COM_midi_in_ep: Used as an IN endpoint number.
    *  COM_midiInBuffer: Function loads the data from this buffer to
    *    the USB IN endpoint.
    *   COM_midiInPointer: Cleared to zero when data are sent.
    *
    * \reentrant
    *  No
    *
    *******************************************************************************/
    void COM_MIDI_IN_EP_Service(void) 
    {
        ...
        if (COM_midiInPointer != 0u)
        {
            if(COM_GetEPState(COM_midi_in_ep) == COM_EVENT_PENDING)
            {
            #if (COM_EP_MANAGEMENT_DMA_AUTO)
                COM_LoadInEP(COM_midi_in_ep, NULL, (uint16)COM_midiInPointer);
            #else
                COM_LoadInEP(COM_midi_in_ep, COM_midiInBuffer,
                                                              (uint16) COM_midiInPointer);
            #endif /* (COM_EP_MANAGEMENT_DMA_AUTO) */
    ...
    *******************************************************************************/
    void COM_MIDI_IN_Service(void) 
    {
        ...
        */
        ...
        if (0u == COM_midiInPointer)
        {
            do
            {
                if (COM_midiInPointer <= (COM_MIDI_IN_BUFF_SIZE - COM_EVENT_LENGTH))
                {
                    /* Check MIDI1 input port for a complete event */
                    m1 = COM_MIDI1_GetEvent();
                    if (m1 != 0u)
                    {
                        COM_PrepareInBuffer(m1, (uint8 *)&COM_MIDI1_Event.msgBuff[0],
                                                                       COM_MIDI1_Event.size, COM_MIDI_CABLE_00);
                    }
                }
                ...
                if (COM_midiInPointer <= (COM_MIDI_IN_BUFF_SIZE - COM_EVENT_LENGTH))
                {
                    ...
                    m2 = COM_MIDI2_GetEvent();
                    if (m2 != 0u)
                    {
                        COM_PrepareInBuffer(m2, (uint8 *)&COM_MIDI2_Event.msgBuff[0],
                                                                       COM_MIDI2_Event.size, COM_MIDI_CABLE_01);
                    }
                }
            #endif /*  COM_MIDI_EXT_MODE >= COM_TWO_EXT_INTRF */
            }
            while((COM_midiInPointer <= (COM_MIDI_IN_BUFF_SIZE - COM_EVENT_LENGTH)) &&
                  ((m1 != 0u) || (m2 != 0u)));
        }
    #endif /* (COM_MIDI_EXT_MODE >= COM_ONE_EXT_INTRF) */

        /* Service the USB MIDI input endpoint */
        COM_MIDI_IN_EP_Service();
    }


    /***************************************************************************
    * Function Name: COM_PutUsbMidiIn
    ************************************************************************//**
    *
    ...
    *  The COM_MIDI_IN_Service() function should also be called to
    ...
    *   ----------------------|-----------------------------------------
    *  COM_TRUE  | Host is not ready to receive this message
    *  COM_FALSE | Success transfer
    *
    * \globalvars
    *
    *  \ref COM_midi_in_ep: MIDI IN endpoint number used for
    *        sending data.
    *
    *  \ref COM_midiInPointer: Checked this variable to see if
    ...
    *
    ***************************************************************************/
    uint8 COM_PutUsbMidiIn(uint8 ic, const uint8 midiMsg[], uint8 cable)
                                                                
    {
        uint8 retError = COM_FALSE;
        uint8 msgIndex;
        ...
        if (COM_midiInPointer >
                    (COM_EP[COM_midi_in_ep].bufferSize - COM_EVENT_LENGTH))
        {
            COM_MIDI_IN_EP_Service();
        }
        ...
        if (COM_midiInPointer <=
                    (COM_EP[COM_midi_in_ep].bufferSize - COM_EVENT_LENGTH))
        {
            if((ic < COM_EVENT_LENGTH) || (ic >= COM_MIDI_STATUS_MASK))
            {
                COM_PrepareInBuffer(ic, midiMsg, ic, cable);
            }
            else
            {
                ...
                do
                {
                    COM_PrepareInBuffer(COM_MIDI_SYSEX, &midiMsg[msgIndex],
                                                     COM_EVENT_BYTE3, cable);

                    ic -= COM_EVENT_BYTE3;
                    msgIndex += COM_EVENT_BYTE3;
                ...
                while (ic > COM_EVENT_BYTE3);

                if (retError == COM_FALSE)
                {
                    COM_PrepareInBuffer(COM_MIDI_EOSEX, midiMsg, ic, cable);
                }
            }
        }
        else
    ...
    /*******************************************************************************
    * Function Name: COM_PrepareInBuffer
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  COM_midiInBuffer: This buffer is used for saving and combine the
    *    received from UART(s) and(or) generated internally by
    *    COM_PutUsbMidiIn() function messages.
    *  COM_midiInPointer: Used as an index for midiInBuffer to
    *     write data.
    *
    * \reentrant
    *  No
    *
    *******************************************************************************/
    void COM_PrepareInBuffer(uint8 ic, const uint8 srcBuff[], uint8 eventLen, uint8 cable)
                                                                 
    {
        ...
        if (ic >= (COM_MIDI_STATUS_MASK | COM_MIDI_SINGLE_BYTE_MASK))
        {
            COM_midiInBuffer[COM_midiInPointer] = COM_SINGLE_BYTE | cable;
            COM_midiInPointer++;
            COM_midiInBuffer[COM_midiInPointer] = ic;
            COM_midiInPointer++;
            COM_midiInBuffer[COM_midiInPointer] = 0u;
            COM_midiInPointer++;
            COM_midiInBuffer[COM_midiInPointer] = 0u;
            COM_midiInPointer++;
        }
        else if((ic < COM_EVENT_LENGTH) || (ic == COM_MIDI_SYSEX))
        {
            if(ic == COM_MIDI_SYSEX)
            {
                COM_midiInBuffer[COM_midiInPointer] = COM_SYSEX | cable;
                COM_midiInPointer++;
            }
            else if (srcBuffZero < COM_MIDI_SYSEX)
            {
                COM_midiInBuffer[COM_midiInPointer] = (srcBuffZero >> 4u) | cable;
                COM_midiInPointer++;
            }
            else if (srcBuffZero == COM_MIDI_TUNEREQ)
            {
                COM_midiInBuffer[COM_midiInPointer] = COM_1BYTE_COMMON | cable;
                COM_midiInPointer++;
            }
            else if ((srcBuffZero == COM_MIDI_QFM) || (srcBuffZero == COM_MIDI_SONGSEL))
            {
                COM_midiInBuffer[COM_midiInPointer] = COM_2BYTE_COMMON | cable;
                COM_midiInPointer++;
            }
            else if (srcBuffZero == COM_MIDI_SPP)
            {
                COM_midiInBuffer[COM_midiInPointer] = COM_3BYTE_COMMON | cable;
                COM_midiInPointer++;
            }
            else
            {
            ...
            COM_midiInBuffer[COM_midiInPointer] = srcBuffZero;
            COM_midiInPointer++;
            COM_midiInBuffer[COM_midiInPointer] = srcBuffOne;
            COM_midiInPointer++;
            COM_midiInBuffer[COM_midiInPointer] = srcBuff[2u];
            COM_midiInPointer++;
        }
        else if (ic == COM_MIDI_EOSEX)
        {
            switch (eventLen)
            {
                case 0u:
                    COM_midiInBuffer[COM_midiInPointer] =
                                                                        COM_SYSEX_ENDS_WITH1 | cable;
                    COM_midiInPointer++;
                    COM_midiInBuffer[COM_midiInPointer] = COM_MIDI_EOSEX;
                    COM_midiInPointer++;
                    COM_midiInBuffer[COM_midiInPointer] = 0u;
                    COM_midiInPointer++;
                    COM_midiInBuffer[COM_midiInPointer] = 0u;
...
*/
#if (COM_MIDI_EXT_MODE >= COM_ONE_EXT_INTRF)
    /*******************************************************************************
    * Function Name: COM_MIDI_InitInterface
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *   COM_MIDI_Event: initialized to zero.
    *   COM_MIDI_TxRunStat: initialized to zero.
    *
    *******************************************************************************/
    void COM_MIDI_InitInterface(void) 
    {
        COM_MIDI1_Event.length  = 0u;
        COM_MIDI1_Event.count   = 0u;
        COM_MIDI1_Event.size    = 0u;
        COM_MIDI1_Event.runstat = 0u;
        COM_MIDI1_TxRunStat     = 0u;
        COM_MIDI1_InqFlags      = 0u;

        /* Start UART block */
        MIDI1_UART_Start();

        /* Change the priority of the UART TX and RX interrupt */
        CyIntSetPriority(MIDI1_UART_TX_VECT_NUM, COM_CUSTOM_UART_TX_PRIOR_NUM);
        CyIntSetPriority(MIDI1_UART_RX_VECT_NUM, COM_CUSTOM_UART_RX_PRIOR_NUM);

    #if (COM_MIDI_EXT_MODE >= COM_TWO_EXT_INTRF)
        COM_MIDI2_Event.length  = 0u;
        COM_MIDI2_Event.count   = 0u;
        COM_MIDI2_Event.size    = 0u;
        COM_MIDI2_Event.runstat = 0u;
        COM_MIDI2_TxRunStat     = 0u;
        COM_MIDI2_InqFlags      = 0u;

        /* Start second UART block */
        MIDI2_UART_Start();

        /* Change the priority of the UART TX interrupt */
        CyIntSetPriority(MIDI2_UART_TX_VECT_NUM, COM_CUSTOM_UART_TX_PRIOR_NUM);
        CyIntSetPriority(MIDI2_UART_RX_VECT_NUM, COM_CUSTOM_UART_RX_PRIOR_NUM);
    #endif /* (COM_MIDI_EXT_MODE >= COM_TWO_EXT_INTRF) */
    ...
    #ifdef COM_MIDI_INIT_CALLBACK
        COM_MIDI_Init_Callback();
    #endif /* (COM_MIDI_INIT_CALLBACK) */
    }


    /*******************************************************************************
    * Function Name: COM_ProcessMidiIn
    ****************************************************************************//**
    *
    ...
    *
    *******************************************************************************/
    uint8 COM_ProcessMidiIn(uint8 mData, COM_MIDI_RX_STATUS *rxStat)
                                                                
    {
        ...
        */
        if ((mData & COM_MIDI_STATUS_BYTE_MASK) != 0u)
        {
            if ((mData & COM_MIDI_STATUS_MASK) == COM_MIDI_STATUS_MASK)
            {
                if ((mData & COM_MIDI_SINGLE_BYTE_MASK) != 0u) /* System Real-Time Messages(single byte) */
                {
                    midiReturn = mData;
                }
                else                              /* System Common Messages */
                {
                    ...
                    {
                        case COM_MIDI_SYSEX:
                            rxStat->msgBuff[0u] = COM_MIDI_SYSEX;
                            rxStat->runstat = COM_MIDI_SYSEX;
                            rxStat->count = 1u;
                            rxStat->length = 3u;
                            break;
                        case COM_MIDI_EOSEX:
                            ...
                            rxStat->count = 0u;
                            midiReturn = COM_MIDI_EOSEX;
                            break;
                        case COM_MIDI_SPP:
                            rxStat->msgBuff[0u] = COM_MIDI_SPP;
                            rxStat->runstat = 0u;
                            rxStat->count = 1u;
                            rxStat->length = 3u;
                            break;
                        case COM_MIDI_SONGSEL:
                            rxStat->msgBuff[0u] = COM_MIDI_SONGSEL;
                            rxStat->runstat = 0u;
                            rxStat->count = 1u;
                            rxStat->length = 2u;
                            break;
                        case COM_MIDI_QFM:
                            rxStat->msgBuff[0u] = COM_MIDI_QFM;
                            rxStat->runstat = 0u;
                            rxStat->count = 1u;
                            rxStat->length = 2u;
                            break;
                        case COM_MIDI_TUNEREQ:
                            rxStat->msgBuff[0u] = COM_MIDI_TUNEREQ;
                            rxStat->runstat = 0u;
                            rxStat->size = 1u;
                            rxStat->count = 0u;
            ...
            else /* Channel Messages */
            {
                ...
                rxStat->count = 1u;
                switch (mData & COM_MIDI_STATUS_MASK)
                {
                    case COM_MIDI_NOTE_OFF:
                    case COM_MIDI_NOTE_ON:
                    case COM_MIDI_POLY_KEY_PRESSURE:
                    case COM_MIDI_CONTROL_CHANGE:
                    case COM_MIDI_PITCH_BEND_CHANGE:
                        rxStat->length = 3u;
                        break;
                    case COM_MIDI_PROGRAM_CHANGE:
                    case COM_MIDI_CHANNEL_PRESSURE:
                        rxStat->length = 2u;
                        break;
                    default:
                        rxStat->runstat = 0u;
        ...
        {
            if (rxStat->runstat == COM_MIDI_SYSEX)
            {
            ...
            else if (rxStat->runstat != 0u)
            {
                ...
                rxStat->count = 2u;
                switch (rxStat->runstat & COM_MIDI_STATUS_MASK)
                {
                    case COM_MIDI_NOTE_OFF:
                    case COM_MIDI_NOTE_ON:
                    case COM_MIDI_POLY_KEY_PRESSURE:
    ...
    /*******************************************************************************
    * Function Name: COM_MIDI1_GetEvent
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  COM_MIDI1_Event: RX status structure used to parse received
    *    data.
    *
    *******************************************************************************/
    uint8 COM_MIDI1_GetEvent(void) 
    {
        uint8 msgRtn = 0u;
        uint8 rxData;
            ...
            */
            ...
            #endif /*  ((MIDI1_UART_RXBUFFERSIZE >= 256u) && (CY_PSOC3)) */
                {
                    ...
                    */
                    ...
                    msgRtn = COM_ProcessMidiIn(rxData,
                                                    (COM_MIDI_RX_STATUS *)&COM_MIDI1_Event);
    ...
    /*******************************************************************************
    * Function Name: COM_MIDI1_ProcessUsbOut
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  COM_MIDI1_TxRunStat: This variable used to save the MIDI
    *    status byte and skip to send the repeated status byte in subsequent event.
    *  COM_MIDI1_InqFlags: The following flags are set when SysEx
    *    message comes.
    *    COM_INQ_SYSEX_FLAG: Non-Real Time SySEx message received.
    *    COM_INQ_IDENTITY_REQ_FLAG: Identity Request received.
    *      This bit should be cleared by user when Identity Reply message generated.
    *
    *******************************************************************************/
    void COM_MIDI1_ProcessUsbOut(const uint8 epBuf[])
    ...
    #ifdef COM_MIDI1_PROCESS_USB_OUT_ENTRY_CALLBACK
        COM_MIDI1_ProcessUsbOut_EntryCallback();
    #endif /* (COM_MIDI1_PROCESS_USB_OUT_ENTRY_CALLBACK) */

        cmd = epBuf[COM_EVENT_BYTE0] & COM_CIN_MASK;

        if ((cmd != COM_RESERVED0) && (cmd != COM_RESERVED1))
        {
            len = COM_MIDI_SIZE[cmd];
            i = COM_EVENT_BYTE1;
            /* Universal System Exclusive message parsing */
            if (cmd == COM_SYSEX)
            {
                if ((epBuf[COM_EVENT_BYTE1] == COM_MIDI_SYSEX) &&
                    (epBuf[COM_EVENT_BYTE2] == COM_MIDI_SYSEX_NON_REAL_TIME))
                {
                    /* Non-Real Time SySEx starts */
                    COM_MIDI1_InqFlags |= COM_INQ_SYSEX_FLAG;
                }
                else
                {
                    COM_MIDI1_InqFlags &= (uint8)~COM_INQ_SYSEX_FLAG;
                }
            }
            else if (cmd == COM_SYSEX_ENDS_WITH1)
            {
                COM_MIDI1_InqFlags &= (uint8)~COM_INQ_SYSEX_FLAG;
            }
            else if (cmd == COM_SYSEX_ENDS_WITH2)
            {
                COM_MIDI1_InqFlags &= (uint8)~COM_INQ_SYSEX_FLAG;
            }
            else if (cmd == COM_SYSEX_ENDS_WITH3)
            {
                /* Identify Request support */
                if ((COM_MIDI1_InqFlags & COM_INQ_SYSEX_FLAG) != 0u)
                {
                    COM_MIDI1_InqFlags &= (uint8)~COM_INQ_SYSEX_FLAG;
                    if ((epBuf[COM_EVENT_BYTE1] == COM_MIDI_SYSEX_GEN_INFORMATION) &&
                        (epBuf[COM_EVENT_BYTE2] == COM_MIDI_SYSEX_IDENTITY_REQ))
                    {
                        /* Set the flag about received the Identity Request.
                        *  The Identity Reply message may be send by user code.
                        */
                        COM_MIDI1_InqFlags |= COM_INQ_IDENTITY_REQ_FLAG;
                    }
                }
            }
            ...
            if ((cmd >= COM_NOTE_OFF) && (cmd <= COM_PITCH_BEND_CHANGE))
            {
                if (COM_MIDI1_TxRunStat == epBuf[COM_EVENT_BYTE1])
                {
                ...
                else
                {
                    ...
                    COM_MIDI1_TxRunStat = epBuf[COM_EVENT_BYTE1];
                }
            }
            else
            {
    ...
    #ifdef COM_MIDI1_PROCESS_USB_OUT_EXIT_CALLBACK
        COM_MIDI1_ProcessUsbOut_ExitCallback();
    #endif /* (COM_MIDI1_PROCESS_USB_OUT_EXIT_CALLBACK) */
    }


#if (COM_MIDI_EXT_MODE >= COM_TWO_EXT_INTRF)
    /*******************************************************************************
    * Function Name: COM_MIDI2_GetEvent
    ****************************************************************************//**
    *
    *  Checks for incoming MIDI data, calls the MIDI event builder if so.
    *  Returns either empty or with a complete event.
    ...
    *
    * \globalvars
    *  COM_MIDI2_Event: RX status structure used to parse received
    *    data.
    *
    *******************************************************************************/
    uint8 COM_MIDI2_GetEvent(void) 
    {
        uint8 msgRtn = 0u;
        uint8 rxData;
            ...
            */
            ...
            #endif /* ((MIDI2_UART_RXBUFFERSIZE >= 256u) && (CY_PSOC3)) */
                {
                    ...
                    */
                    ...
                    msgRtn = COM_ProcessMidiIn(rxData,
                                                    (COM_MIDI_RX_STATUS *)&COM_MIDI2_Event);
    ...
    /*******************************************************************************
    * Function Name: COM_MIDI2_ProcessUsbOut
    ****************************************************************************//**
    *
    *  Process a USB MIDI output event.
    *  Puts data into the MIDI TX output buffer.
    ...
    *
    * \globalvars
    *  COM_MIDI2_TxRunStat: This variable used to save the MIDI
    *    status byte and skip to send the repeated status byte in subsequent event.
    *  COM_MIDI2_InqFlags: The following flags are set when SysEx
    *    message comes.
    *  COM_INQ_SYSEX_FLAG: Non-Real Time SySEx message received.
    *  COM_INQ_IDENTITY_REQ_FLAG: Identity Request received.
    *    This bit should be cleared by user when Identity Reply message generated.
    *
    *******************************************************************************/
    void COM_MIDI2_ProcessUsbOut(const uint8 epBuf[])
                                                            
    {
    ...
    #ifdef COM_MIDI2_PROCESS_USB_OUT_ENTRY_CALLBACK
        COM_MIDI2_ProcessUsbOut_EntryCallback();
    #endif /* (COM_MIDI2_PROCESS_USB_OUT_ENTRY_CALLBACK) */
        ...
        if ((cmd != COM_RESERVED0) && (cmd != COM_RESERVED1))
        {
            ...
            if(cmd == COM_SYSEX)
            {
                ...
                {
                    /* SySEx starts */
                    COM_MIDI2_InqFlags |= COM_INQ_SYSEX_FLAG;
                }
                else
                {
                    COM_MIDI2_InqFlags &= (uint8)~COM_INQ_SYSEX_FLAG;
                }
            }
            else if(cmd == COM_SYSEX_ENDS_WITH1)
            {
            ...
            {
                /* Identify Request support */
                if ((COM_MIDI2_InqFlags & COM_INQ_SYSEX_FLAG) != 0u)
                {
                    COM_MIDI2_InqFlags &= (uint8)~COM_INQ_SYSEX_FLAG;
                    ...
                    {   /* Set the flag about received the Identity Request.
                        *  The Identity Reply message may be send by user code.
                        */
                        COM_MIDI2_InqFlags |= COM_INQ_IDENTITY_REQ_FLAG;
                    }
                }
            }
            else /* Do nothing for other command */
            ...
            if ((cmd >= COM_NOTE_OFF) && ( cmd <= COM_PITCH_BEND_CHANGE))
            {
                if (COM_MIDI2_TxRunStat == epBuf[COM_EVENT_BYTE1])
                {   /* Skip the repeated Status byte */
                    i++;
                }
                else
                {   /* Save Status byte for next event */
                    COM_MIDI2_TxRunStat = epBuf[COM_EVENT_BYTE1];
                }
            }
            else
            {   /* Clear Running Status */
    ...
    #ifdef COM_MIDI2_PROCESS_USB_OUT_EXIT_CALLBACK
        COM_MIDI2_ProcessUsbOut_ExitCallback();
    #endif /* (COM_MIDI2_PROCESS_USB_OUT_EXIT_CALLBACK) */
    }
#endif /* (COM_MIDI_EXT_MODE >= COM_TWO_EXT_INTRF) */
#endif /* (COM_MIDI_EXT_MODE >= COM_ONE_EXT_INTRF) */

#endif  /*  (COM_ENABLE_MIDI_API != 0u) */
...
#endif  /* defined(COM_ENABLE_MIDI_STREAMING) */
